...
import { collection, query, orderBy, getDocs, limit, startAfter, DocumentData, onSnapshot } from "firebase/firestore";
...
export default function NotifikasiPage() {
    ...
    const getStatusIcon = (status: NotificationJob['status']) => {
        switch (status) {
            ...
            case 'pending': return <Clock className="h-4 w-4 mr-2 animate-spin" />;
                                ...
                                {isLoading ? (
                                    ...
                                    jobs.map(job => (
                                        <TableRow key={job.id}>
                                            ...
                                            <TableCell>
                                                 <p className="max-w-xs truncate text-xs" title={job.errorMessage || job.payload.message}>
